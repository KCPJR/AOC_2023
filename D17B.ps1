    ...
    Begin{
        ...
        switch ($PsCmdlet.ParameterSetName) {
            A {
                ...
                #$data = Get-Content .\Day_17\Input_testb.txt
            }
            B {
        ...
        $maxnodeind = $nodes.count -1
        $scores[$maxnodeind] = 987654321
                ...
                Do{
                    ...
                    If( ($n -ge 0) -and 
                        ...
                        (($prevdir -in ("N","Z") -and $prevcr -le $mr )-or (($prevdir -ne "N") -and $prevcr -ge 4)) -and 
                        ($flagprev)
                        ){
                        $pots += [PSCustomObject]@{
                            N = $n
                    ...
                    If(($n -lt ($nodes.count)) -and 
                        ($n -notin $prevpath)-and 
                        #($prevcr -le $mr -or ($prevdir -ne "S")) -and 
                        (($prevdir -in ("S","Z") -and $prevcr -le $mr )-or (($prevdir -ne "S") -and $prevcr -ge 4)) -and
                        $flagprev){
                        $pots += [PSCustomObject]@{
                            N = $n
                            Dir = "S"
                    ...
                    If( ([math]::Floor(($CN + 1 ) / $mod) -eq [math]::Floor(($CN) / $mod)) -and 
                        ($n -notin $prevpath) -and 
                        (($prevdir -in ("E","Z") -and $prevcr -le $mr )-or (($prevdir -ne "E") -and $prevcr -ge 4)) -and
                        #($prevcr -le $mr -or ($prevdir -ne "E")) -and 
                        $flagprev){
                        $pots += [PSCustomObject]@{
                            N = $n
                            Dir = "E"
                    ...
                    If(([math]::Floor(($CN - 1 ) / $mod) -eq [math]::Floor(($CN) / $mod)) -and 
                        ($n -notin $prevpath) -and 
                        (($prevdir -in ("W","Z") -and $prevcr -le $mr )-or (($prevdir -ne "W") -and $prevcr -ge 4)) -and
                        #($prevcr -le $mr -or ($prevdir -ne "W")) -and 
                        $flagprev){
                        $pots += [PSCustomObject]@{
                            N = $n
                            Dir = "W"
...
########################                        
                        ...
                        Else{
                            if(!$scores.ContainsKey($pot.N)){
                                $scores.Add($pot.n,$newscore)
                            }elseif($scores[$pot.N] -gt $newscore){$scores[$pot.N] = $newscore}
                    ...
                    $CN = $lowest
                    if($newstates|?{$_.Index -eq $maxnodeind})
                        {Write-host("Pause")}
                }
                while(($scores[$maxnodeind] -eq 987654321) )
                #while($lowest -ne ($nodes.count -1) -and ($scores[$maxnodeind] -eq 987654321) )
                #while($lowest -ne ($nodes.count -1) -and $k -le 1000 )
                
                #$newstates
                #$states
                Write-host ("Solution Part 2: {0}" -f $scores[$nodes.count -1])
                write-host ("Pause")
