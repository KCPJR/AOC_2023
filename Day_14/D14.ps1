Function D14{
    ...
    Begin{
        ...
        }#movement complete for a column
        $newmaprotated += ,($tempstr.ToCharArray())
        
        $tsc = $tempstr.ToCharArray()
        $colscore = 0
        for($j = 0;$j -le $maxrowindex;$j++){
            if($tsc[$j] -eq "O"){$colscore += $maxrowindex + 1 -$j}
        }
        write-host (" Col Score = {0}" -f $colscore)
        $totscore += $colscore
        ...
        write-host("Answer Part 1: {0}" -f $totscore)
    }
}